...
fun getHoursAgo(date: Date): String {
    return try {
        ...
        val dateFormat = "dd-MMM-yyyy hh:mm:ss a"
        val formatter = SimpleDateFormat(dateFormat, Locale.ENGLISH)
        formatter.timeZone = TimeZone.getTimeZone("UTC")

        val newDate: Date = formatter.parse(DateFormat.getDateTimeInstance().format(date))!!
        formatter.timeZone = TimeZone.getDefault()
        val ago = DateUtils.getRelativeTimeSpanString(newDate.time, now, DateUtils.MINUTE_IN_MILLIS)
        ago.toString()
